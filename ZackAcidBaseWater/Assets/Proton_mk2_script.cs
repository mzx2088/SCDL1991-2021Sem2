...
public class Proton_mk2_script : MonoBehaviour
{
    public Transform myParent; //Just to check!
    public float protonSpeed = 1.2f;
    private H_mk2_script destinationScript;

    // Start is called before the first frame update
    void Start()
    {
        myParent = transform.parent;
        destinationScript = GetComponentInParent<H_mk2_script>();
    ...
    private void FixedUpdate()
    {
        ...
        if (separation < 0.1f)
        {
            Transform siblingOrbital = myParent.Find("sp3mk2");
